...
begin
  ...
  if (not (FIdent is TLapeTree_VarType)) and
     (FIdent is TLapeTree_GlobalVar) and
     (TLapeTree_GlobalVar(FIdent).GlobalVar <> nil) and
     (TLapeTree_GlobalVar(FIdent).GlobalVar.VarType is TLapeType_Type)
  then
  ...
  if (not isEmpty(Self)) and (RealIdent = nil) and (FIdent is TLapeTree_GlobalVar) then
    ...
    begin
      if (GlobalVar <> nil) and (VarType <> nil) and (VarType is TLapeType_OverloadedMethod) then
        Method := TLapeType_OverloadedMethod(VarType).getMethod(getParamTypes(), ExpectType)
      else
        Method := nil;
      if (Method <> nil) then
        Ident := TLapeTree_GlobalVar.Create(Method, Ident);
    end
end;
...
begin
  if (FRes = nil) and (FIdent <> nil) then
    if (FIdent is TLapeTree_VarType) then
      FRes := DoCast()
    else
    begin
    ...
    begin
      ...
      try
        if (ParamVars[i].VarPos.MemPos = NullResVar.VarPos.MemPos) then
          if (Params[i].ParType in Lape_RefParams) or ((Params[i].VarType <> nil) and (Params[i].VarType.NeedFinalization)) then
...
function TLapeTree_InternalMethod_Operator.resType: TLapeType;
var
...
begin
  ...
  begin
    ...
    _LapeOperator := FCompiler['!op_'+op_name[FOperator]];
    if _LapeOperator <> nil then
    begin
      methodVar := TLapeTree_GlobalVar.Create(_LapeOperator, Self);
...
function TLapeTree_InternalMethod_Operator.Compile(var Offset: Integer): TResVar;
var
  _LapeOperator: TLapeGlobalVar;
...
begin
  if (FRes = nil) then
    if (FParams.Count <> 1) or isEmpty(FParams[0]) then
      ...
      setIdent(TLapeTree_VarType.Create(resType(), Self));
  Result := inherited;
end;
